...
\begin{itemize}
%  \item We recap asynchronous concepts as a specification
%  language in Section~\ref{sec:Concepts}.
  \item We detail some necessary concepts, required for an accurate 
  translation in Section~\ref{sec:concept_types}
...
%  \item We display the design flow with a small example in our open-source 
%  software toolsuite \noun{Workcraft}, Section~\ref{sec:interop-with-stg}.
\end{itemize}

\noindent
We start with a brief recap of asynchronous concepts as a specification language in
Section~\ref{sec:Concepts}, and discuss the design flow using the tool within our
...
\subsection{Concepts for asynchronous
circuits\label{sub:Concepts-for-asynchronous}}
...
\label{sec:Circuit-specification-with}}
...
\end{algorithmic}
\end{algorithm}
%A method of synthesizing concepts directly also exists, which will be
...
%which can be tested and verified separately to allow for easier changes and
additions
%to smaller, less complex, specifications. However, when scenarios are tested
and prove to work as required, and satisfy all necessary properties, they then
need to be combined to
%produce a full system specification, so this can be tested and verified, and
...
%When we have acquired a circuit design via a chosen synthesis type, it needs to
be verified to
